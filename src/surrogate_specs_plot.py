...
def main(device):
    ...
    dualview_root="/home/fe/yolcu/Documents/Code/DualView-wip"
    results=f"{dualview_root}/results/C_plot_results.dict"
    results_dict=torch.load(results) if os.path.exists(results) else {}
    ...
    if results_dict=={}:
        for i, dataset_name in enumerate(["MNIST", "CIFAR", "AWA"]):
            ...
            root=f"{dualview_root}/cache/{dataset_name}/std"
            outdir="/mnt/outputs"
            outdir=f"{dualview_root}/test_outputs"
            ...
            if dataset_name not in results_dict.keys():
                ...
                train_accs=torch.tensor(train_accs, device=device)
                test_accs=torch.tensor(test_accs, device=device)
                train_times=torch.tensor(train_times, device=device)
                results_dict[dataset_name]["sv_counts"]=sv_counts
                results_dict[dataset_name]["train_accs"]=train_accs
                results_dict[dataset_name]["test_accs"]=test_accs
                results_dict[dataset_name]["train_times"]=train_times
...
# Turns off grid on the secondary (right) Axis.
        ...
        fig.tight_layout()
    plt.savefig(f"{outdir}/C_plot.png", dpi=300, bbox_inches='tight')
