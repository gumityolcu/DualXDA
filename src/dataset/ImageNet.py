...
IMAGENET_CLASS_DICT = {0: 'tench, Tinca tinca',
...
class ImageNet(tvImageNet):
    ...
    name='ImageNet'
    class_name_dict = IMAGENET_CLASS_DICT
    default_transform = VGG16_Weights.IMAGENET1K_V1.transforms()
...
# ])
    ...
    def __getitem__(self, item):
        if self.val_split=="train":
            assert item < int(super(tvImageNet, self).__len__() * 0.96), f"index {item} out of range for train set"
            train_item = self.get_index(item, "train")
            im, label = super(tvImageNet, self).__getitem__(train_item)
        else:
            assert item < int(super(tvImageNet, self).__len__() * 0.04), f"index {item} out of range for val set"
    ...
    def __len__(self):
        if self.val_split=="train":
            return int(super(tvImageNet, self).__len__() * 0.96)
        else:
            return int(super(tvImageNet, self).__len__() * 0.04)