...
from lxt.utils import _generate_latex
import torch
from transformers import AutoTokenizer
...
def _compile_latex_to_pdf(latex_code, path='word_colors.pdf', delete_aux_files=True, backend='xelatex'):
    ...
    if backend == 'pdflatex':
        subprocess.call(['pdflatex', '-interaction=nonstopmode', '--output-directory', path.parent, path.with_suffix(".tex")])
    elif backend == 'xelatex':
        subprocess.call(['xelatex', '-interaction=nonstopmode', '--output-directory', path.parent, path.with_suffix(".tex")])
