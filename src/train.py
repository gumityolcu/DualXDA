...
def start_training(model_name, device, num_classes, class_groups, data_root, epochs,
    ...
    if model_name == "resnet18":
        model = load_model(model_name, dataset_name, num_classes, train=True).to(device)
    elif model_name == "resnet50":
        model = load_awa_model(model_name, dataset_name, num_classes, train=True).to(device)
    else:
