...
# Filter to only include checkpoints 7, 75, and 750
df = df[df[df.columns[1]].isin([7, 15, 37, 75, 150, 375, 750, 1500])]
...
grouped = df.groupby(['group', df.columns[1]]).agg({  # Assuming second column is checkpoint
    df.columns[2]: ['mean', 'std', 'count']  # Third column is loss
}).reset_index()

# Flatten column names
grouped.columns = ['group', 'checkpoint', 'mean_loss', 'std_loss', 'count']

# Calculate 95% confidence interval
grouped['ci_95'] = grouped.apply(
    lambda row: stats.t.ppf(0.975, row['count']-1) * row['std_loss'] / np.sqrt(row['count']) 
...
plt.title('Model Loss vs Checkpoint (Mean ± 95% CI)', fontsize=14, fontweight='bold')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("loss_vs_step_grouped_err.png")
plt.show()
...
for group in groups:
    ...
    print(f"  Loss range: {group_data.iloc[:, 2].min():.4f} - {group_data.iloc[:, 2].max():.4f}")